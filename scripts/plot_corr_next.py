...
def run_next_corr(df, unique_syllables, bird_num, save_mode="default", save_folder=None, column="song"):
    ...
    if save_mode != "synth":
        ...
        corr_df_mod = corr_df.copy()

        def rename_label(label):
            return label.upper()

        corr_df_mod.index = [rename_label(lbl) for lbl in corr_df_mod.index]
        corr_df_mod.columns = [rename_label(lbl) for lbl in corr_df_mod.columns]

        # Create heatmap_data and masks
        heatmap_data = pd.DataFrame(index=unique_syllables, columns=unique_syllables, dtype=float)
        ...
        for s1 in unique_syllables:
            for s2 in unique_syllables:
                ...
                if n < 100:
                    heatmap_data.loc[s1, s2] = np.nan
                elif p < alpha and n >= 100:
                    heatmap_data.loc[s1, s2] = r
                else:
                    heatmap_data.loc[s1, s2] = np.nan
                    grey_mask.loc[s1, s2]    = True
        ...
        plt.title(f"Bird {bird_num}: A to first next B\n"
        ...
        plt.tight_layout()
        plt.subplots_adjust(top=0.8, bottom=0.2, left=0.1, right=0.75)
        plt.show()
